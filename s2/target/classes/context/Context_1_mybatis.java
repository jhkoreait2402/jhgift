...
public class Context_1_mybatis {
	...
	// 이설정은 스프링이 SqlSessionFactoryBean 자체를 생성하는 것이 아니라
	// Factory에서 getObject()메서드를 호출한 결과를 리턴한다.
	// 이 경우 스프링은 어플리케이션 시작 시점에서 SqlSessionFactory를 빌드하고
	...
	// 왜나하면, 스프링 트랜잭션 설정에 따라 자동으로 커밋 또는 롤백을 수행하기 때문이다.
	
	// SqlSessionTemplate클래스는 SqlSession을 implements하고 코드에서 SqlSession을 대체하는 역할을 한다.
	// SQL을 처리하는 Mybatis Method를 호출할 때 SqlSession이 현재의 Spring tracsaction에서 사용될 수 있도록 보장해준다.
	// SqlSessionTemplate은 필요한 시점에서 Session을 닫고, Commit하거나 Rollback하는 것을 포함한 Session의 라이프사이클을 관리한다.
